...
##Improvement: use tapply function to calculatethe mean city and highway mileage for each class of car.
#cty_mean<-tapply(mpg$cty,mpg$class,mean)
#hwy_mean<-tapply(mpg$hwy,mpg$class,mean)
...
##2. Highway MPG by Class of Car

```{r,echo=FALSE} 
 ggplot(mpg) +geom_point(mapping =aes(x = displ, y = hwy,color=class),size=2) + geom_smooth(mapping =aes(x = displ, y = hwy)) +
 scale_color_hue() +
 theme_minimal(base_size = 16) +
 facet_wrap(vars(year)) + 
